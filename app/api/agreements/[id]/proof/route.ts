...
export async function GET(
  ...
  const { data: agreement, error: aErr } = await supabase
    .from("agreements")
    .select(
      "id, creator_id, status, signed_at, finalized_version_id, current_version_id"
    )
    .eq("id", id)
    .maybeSingle();
  ...
  const signers: FinalProofSignerEntry[] = (sigRows ?? []).map((s: any) => ({
    ...
    key_version:
      typeof s.signer_key_version === "number" ? s.signer_key_version : null,
    passkey_credential_id: s.webauthn_credential_id ?? null,
  }));

  const signedAt =
    (agreement.signed_at as string | null) ?? new Date().toISOString();
  ...
  let signerPublicKeys: Record<
    string,
    { public_key_pem: string; key_version: number }
  > = {};
  try {
  ...
  const exportJson = {
    ...
    signatures: (sigRows ?? []).map((s: any) => ({
      ...
      signer_public_key_pem:
        signerPublicKeys[s.signer_id]?.public_key_pem ?? null,
    })),
    anchor,
  };
